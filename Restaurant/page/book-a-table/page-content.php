<div class="container">
	...
	<div class="head-book-a-table room_no">
		...
		<div class="row">
			...
			?>
			<a class="pull-left button btn btn-primary room_nr room_no_<?php echo $query_room['restaurant_room_nr']; ?>">Food Category. <?php echo $query_room['restaurant_room_nr']; ?><i class="fa fa-check"></i></a>
			<?php } ?>
		</div>
	</div>
...
if (loggedIn()) {
	...
	if (isset($_POST['order_date'])){
		...
		if(isset($_POST["submit_order_book_table"])){
			...
			echo "<div class='container'><h2>" . $lang['Reservation_for'] . ": <strong>" . $_POST['table_details'] . "</strong>," . $lang['Reservation_date'] . "<strong>" . $date->format( 'd-m-Y') . " </strong>, " . $lang['Reservation_time'] . ": <strong>" . $date->format( 'H:i') . "</strong> " . $lang['Reservation_done'] . " <br />" . $lang['Reservation_time_to_stay'] . "<strong class='text-center'>" ?> <?php if ($_POST['number_of_hours'] == '1') { echo $lang['one_hour'];}else{echo $_POST['number_of_hours'] . " " . $lang['hours'];} ?> <?php echo "</strong>. <strong class='red'>" . $lang['success_message'] . "<strong></h2></div>";
			...
			$query_orders = "INSERT INTO orders (order_type,booktable_room,booktable_tables,order_user_name, order_user_email, order_user_phone,order_date,book_date_out) VALUES ('$order_type','$booktable_room','$table_details', '$order_user_nice_name', '$order_user_email', '$order_user_phone', '$order_date', '$book_date_out');";
...
<!-- BOF: .submit_book_a_table -->
		<div class="container700">
			<form method="POST" class="container submit_book_a_table">
				...
				<div class="label_input_group">
					...
					<div class="col-md-6">
						<input required class="form-control" type='text' name='order_user_nice_name' placeholder="<?php echo $lang['Name_and_surname']; ?>" value="<?php if (loggedIn()) { echo $order_user_nice_name; } ?>">
					</div>
				</div>
				<div class="label_input_group">
					<label class="col-md-6"><?php echo $lang['Email']; ?></label>
					<div class="col-md-6">
						<input required class="form-control" type='text' name='order_user_email' placeholder="<?php echo $lang['Email']; ?>" value="<?php if (loggedIn()) { echo $order_user_email; } ?>" >
					</div>
				</div>
				<div class="label_input_group">
					<label class="col-md-6"><?php echo $lang['Phone_number']; ?></label>
					<div class="col-md-6">
						<input required class="form-control" type='text' name='order_user_phone' placeholder="<?php echo $lang['Phone_number']; ?>" value="<?php if (loggedIn()) { echo '0' . $order_user_phone; } ?>" >
					</div>
				</div>
				<div class="row label_input_group">
				...
				<input class="pull-right btn btn-success" type="submit" name="submit_order_book_table" value="<?php echo $lang['Book_now']; ?>" />
